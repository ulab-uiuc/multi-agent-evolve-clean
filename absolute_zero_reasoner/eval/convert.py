...
#         eval_csv_path = os.path.join(model_path, "eval_results", "eval_results.csv")
#         if not os.path.isfile(eval_csv_path):
#             print(f"⚠️ Missing: {eval_csv_path}")
#             continue

#         with open(eval_csv_path, newline='') as csvfile:
#             reader = csv.DictReader(csvfile)
#             for row in reader:
#                 result = OrderedDict()
#                 result["model"] = model_dir
#                 try:
#                     # result['hmmt_2024_acc'] = row.pop('hmmt_2024_acc')
#                     result['minerva_math_acc'] = row.pop('minerva_math_acc')
#                 except KeyError:
#                     print(f"⚠️ Missing keys in {model_dir}: {row.keys()}")
#                     pass
#                     # print(row.keys())

#                 prefix_to_fields = defaultdict(dict)
#                 # print(row)
#                 for key in list(row.keys()):
#                     # if "minvera" in key:
#                     #     prefix = "minvera_math"
#                     #     if key == f"{prefix}_acc":
#                     #         result[key] = row.pop(key)
#                     #     # else:
#                     # if "hmmt_2024" in key:
#                     #     prefix = "hmmt_2024"
#                     #     if key == f"{prefix}_acc":
...
#                             result[key] = row.pop(key)
#                         else:
#                             prefix_to_fields[prefix][key] = row.pop(key)

#                 # Sort and attach _details
#                 # for prefix in sorted(prefix_to_fields.keys()):
#                 #     details_key = f"{prefix}_details"
#                 #     result[details_key] = {
#                 #         k: prefix_to_fields[prefix][k]
#                 #         for k in sorted(prefix_to_fields[prefix])
#                 #     }
#                 result = insert_sorted_acc_fields(result)

#                 all_results.append(result)

#     # latex = convert_latex_table(all_results, selected_data=['aime24', 'aime25', 'amc23', 'hmmt_2024', 'minerva_math'])
#     # print(latex)
#     # ranks = compute_method_ranks(all_results, selected_models=['azr', 'azr_think'])
#     # print(ranks)
...
def collect_eval_results_by_prefix(root):
    ...
    print(f"✅ Saved structured JSON to {output_path}")
# Example usage
collect_eval_results_by_prefix("/home/fit/huangg/WORK/zqc/eval_baselines/simpleRL-reason/examples/simplelr_math_eval/EVAL/checkpoints")